...
import org.apache.spark.sql.types.{ArrayType, NullType, StringType, StructType, TimestampType, _}
import org.bson.{BsonDocument, _}
...
object MongoMapFunctions {
  /**
    ...
    * @return
    */
  def rowToDocumentMapper(schema: StructType, extendedBsonTypes: Boolean): (Row) => BsonDocument = {
    MapFunctions.rowToDocumentMapper(schema, extendedBsonTypes)
  }
  ...
  /**
    ...
    * @return
    */
  def documentToRowMapper(schema: StructType, requiredColumns: Array[String] = Array.empty[String]): (BsonDocument) => Row = {
    (bsonDocument: BsonDocument) => documentToRow(bsonDocument, schema, requiredColumns)
  }
